...
from PythonQt.paraview import vvCalibrationDialog, vvCropReturnsDialog, vvSelectFramesDialog, vvConnectInformationDialog, vvDataPortSettingDialog
...
def getSaveFileName(title, extension, defaultFileName=None):
    ...
    defaultDir = settings.value('RSlidar16Plugin/OpenData/DefaultDir', QtCore.QDir.homePath())
    ...
    if fileName:
        settings.setValue('RSlidar16Plugin/OpenData/DefaultDir', QtCore.QFileInfo(fileName).absoluteDir().absolutePath())
        return fileName


def restoreNativeFileDialogsAction():
    settings = getPVSettings()
    app.actions['actionNative_File_Dialogs'].setChecked(int(settings.value('RSlidar16Plugin/NativeFileDialogs', 1)))


def onNativeFileDialogsAction():
    settings = getPVSettings()
    defaultDir = settings.setValue('RSlidar16Plugin/NativeFileDialogs', int(app.actions['actionNative_File_Dialogs'].isChecked()))
...
def onDeveloperGuide():
    ...
    paths = ['../doc/RSView_Developer_Guide.pdf']
...
def onUserGuide():
    ...
    paths = ["../doc/RS-Lidar-16User\'s_Guide_v3.3.pdf"]
...
def onAbout():
    title = 'About RSView'
    text = '''<h1>RSView %s</h1><br/>Copyright (c) 2017, Robosense<br />
           Sample Data Repository: show later...'''% getVersionString()
...
def actionAction_udp_reset_deal():
    sensor = getSensor()
    if not sensor:
        QtGui.QMessageBox.warning(getMainWindow(), "warning", "Please open sensor!")
        return

    InputDataSend = [0] * 512
    ii = 0
    csvfiledir = QtGui.QFileDialog.getOpenFileName(getMainWindow(),"Open 512 File",app.csvfiledircent,"CSV files(*.csv)")    
    if csvfiledir == '': 
        distancedirflag = 0
    else:
        distancedirflag = 1
        csvfile = file(csvfiledir, 'rb')
        app.csvfiledircent = csvfiledir
        text = csv.reader(csvfile)
        for line in text:
            InputDataSend[ii] = int(line[0])
            ii = ii + 1
        csvfile.close()
    connect_sdd = sensor.GetClientSideObject().send_Parameter(InputDataSend)
    if connect_sdd==1:
        QtGui.QMessageBox.warning(getMainWindow(), "warning", "connect error!")
...
def onTrailingFramesChanged(numFrames):
    if getReader():
        ...
        try:
            app.sensor.NumberOfTrailingFrames = numFrames
            smp.Render()
            smp.Render(getSpreadSheetViewProxy())
        except AttributeError:
            pass
...
def onFreChanged(fre):
    if getReader():
        try:
            app.reader.freFrames = fre
            smp.Render()
            smp.Render(getSpreadSheetViewProxy())
        except AttributeError:
            pass
...
def onTimeChanged():
    ...
    sensor = getSensor()
    if reader:
        cur_lose = str(reader.GetClientSideObject().return_lose_point_now())
        app.lose.setText('  lose: %s per' % cur_lose)
        cur_framerate = str(reader.GetClientSideObject().return_frame_rate_now())
        #app.framerate.setText('  framerate: %s ' % app.sensor.NumberOfTrailingFrames)
    elif sensor:
        cur_lose = str(sensor.GetClientSideObject().return_lose_point_now())
        app.lose.setText('  lose: %s per' % cur_lose)
        cur_framerate = str(sensor.GetClientSideObject().return_frame_rate_now())
        app.framerate.setText('  framerate: %s ' % app.zhenlv_cur)
...
def onLaserSelection(show = True):
    ...
    actions_table = dialog.findChildren('QTableWidget')
    dialogactions_table = {}
    for a in actions_table:
        dialogactions_table[a.objectName] = a
    dialogactions_table['LaserTable'].itemChanged.connect(onUpdateLaserselection)  

def onDataPortSetting(Show = True):

    dialog = vvDataPortSettingDialog(getMainWindow())
    actions = dialog.findChildren('QPushButton')
    dialogactions = {}
    for a in actions:
        dialogactions[a.objectName] = a
    dialogactions['SetPCPortButtons'].clicked.connect(onSetPCPort)


def onConnectInformation(Show = True):
    ...
    for a in actions:
        dialogactions[a.objectName] = a
    dialogactions['SetButtons'].clicked.connect(onSetUCWP)
    ...
    def onpqUpdate():
        ...
        if reader:
            isValidIP = reader.GetClientSideObject().OutputDIFOP(0)
            if isValidIP==0:  #not a sufficient condition
                gotoNext()
            for i in range(29):
                IPData[i] = reader.GetClientSideObject().OutputDIFOP(i)
        elif sensor:
            for i in range(29):
                IPData[i] = sensor.GetClientSideObject().OutputDIFOP(i)

        for i in range(29):
            dialog.UpdateIP(IPData[i],i)
        dialog.onGetRSPacketIP()

    dialogactions['GetIPButtons'].clicked.connect(onpqUpdate)
    dialog.show()
...
def onSetUCWP():
    ...
    for i in range(29):
       InputData[i] = dialog.OutputInf(i)

    if reader:
        QtGui.QMessageBox.warning(getMainWindow(), "warning", "Can not send UCWP in reading mode")
    elif sensor:
        for i in range(29):
            sensor.GetClientSideObject().InputSetDIFOPData(InputData[i],i)
        if not sensor.GetClientSideObject().SendUCWP():
            QtGui.QMessageBox.information(getMainWindow(), "information", "Send UCWP Packet Successful! Please do not switch off the power immediately!")
        else:
            QtGui.QMessageBox.warning(getMainWindow(), "warning", "Invalid Connection!")


def onDataPortSetting():    
    dialog = vvDataPortSettingDialog(getMainWindow())

    def onSetPCPort():
        PCPortNew = dialog.OutputPCPortNew()
        ...
        if not sensor:
            ...
            if PCPortNew<1024 or PCPortNew>65535:
                QtGui.QMessageBox.warning(getMainWindow(), "warning", "Invalid PC Port!")
                return
            sensor.Stop()
            sensor.GetClientSideObject().SetPCPort(PCPortNew)
            sensor.Start()
            QtGui.QMessageBox.information(getMainWindow(), "information", "Reset PC Port successful!")
            PCPortOld = sensor.GetClientSideObject().GetPCPort()
            dialog.Update(PCPortNew)
...
def setupActions():
    ...
    app.actions['actionAction_udp_reset'].connect('triggered()', actionAction_udp_reset_deal)
    app.actions['actionConnect_Information'].connect('triggered()', onConnectInformation)
    app.actions['actionData_Port_Setting'].connect('triggered()', onDataPortSetting)
    ...
    app.actions['actionRSViewDeveloperGuide'].connect('triggered()', onDeveloperGuide)
    ...
    freBoxLabel = QtGui.QLabel('Fre:')
    freBoxLabel.toolTip = "Fequence"
    timeToolBar.addWidget(freBoxLabel)

    freBox = QtGui.QSpinBox()
    freBox.toolTip = "Fequence"
    freBox.setMinimum(1)
    freBox.setMaximum(10000)
    freBox.setValue(5)
    freBox.connect('valueChanged(int)', onFreChanged)
    app.freBox = freBox

    app.actions['actionfreSelector'] = timeToolBar.addWidget(freBox)
    app.actions['actionfreSelector'].setVisible(True)
